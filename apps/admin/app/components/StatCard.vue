...
const palette: Record<NonNullable<Props['color']>, { bg: string; fg: string }> = {
  blue: { bg: 'bg-primary-50', fg: 'text-primary-600' },
  ...
  slate: { bg: 'bg-ink-100', fg: 'text-ink-600' },
}
</script>

<template>
  <div class="bg-white rounded-xl border border-ink-200 p-5 hover:shadow-sm transition-shadow">
    <div class="flex items-start justify-between gap-3">
      <div class="min-w-0">
        <p class="text-xs font-medium text-ink-500 uppercase tracking-wide truncate">{{ label }}</p>
        <p class="text-2xl font-bold text-ink-900 mt-1.5 truncate">{{ value }}</p>
        <p
          v-if="trend"
