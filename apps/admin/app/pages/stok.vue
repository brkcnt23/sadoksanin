  ...
  <div class="space-y-5">
    ...
    >
      <template #actions>
        <button
          ...
          class="px-3 py-2 text-sm font-medium text-white bg-blue-600 hover:bg-blue-700 rounded-md flex items-center gap-2 disabled:opacity-50"
        >
          <Icon
    ...
    <div class="bg-white rounded-xl border border-slate-200 p-5">
      <h3 class="font-semibold text-slate-900 mb-4 flex items-center gap-2">
        <Icon name="lucide:refresh-cw" class="w-4 h-4 text-blue-600" />
        Netsis Senkronizasyon Durumu
      </h3>
      <dl class="grid grid-cols-2 md:grid-cols-4 gap-4 text-sm">
        <div>
          <dt class="text-xs text-slate-500 uppercase tracking-wider">Son Eşitleme</dt>
          <dd class="font-medium text-slate-900 mt-1">
            {{ stock.syncStatus.lastSyncAt ? formatRelative(stock.syncStatus.lastSyncAt) : 'Hiç' }}
          </dd>
        </div>
        <div>
          <dt class="text-xs text-slate-500 uppercase tracking-wider">Eşitlenen Ürün</dt>
          <dd class="font-medium text-slate-900 mt-1">{{ formatNumber(stock.syncStatus.productsSynced) }}</dd>
        </div>
        <div>
          <dt class="text-xs text-slate-500 uppercase tracking-wider">Hata</dt>
          <dd
            class="font-medium mt-1"
            :class="stock.syncStatus.errors > 0 ? 'text-red-600' : 'text-slate-900'"
          >
            {{ stock.syncStatus.errors }}
          </dd>
        </div>
        <div>
          <dt class="text-xs text-slate-500 uppercase tracking-wider">Sonraki Eşitleme</dt>
          <dd class="font-medium text-slate-900 mt-1">
    ...
    <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
      ...
      <div class="bg-white rounded-xl border border-slate-200">
        <div class="px-5 py-4 border-b border-slate-200 bg-orange-50">
        ...
        <div v-if="lowStock.length > 0" class="divide-y divide-slate-100">
          <div v-for="p in lowStock" :key="p.id" class="px-5 py-3 flex items-center justify-between bg-orange-50">
            <div class="min-w-0 flex-1">
              <p class="font-medium text-sm text-slate-900 truncate">{{ p.name }}</p>
              <p class="text-xs text-slate-500 font-mono">{{ p.sku }} · Min: {{ p.minimumStock }}{{ p.middleStock ? ` / Ort: ${p.middleStock}` : '' }}</p>
            </div>
      ...
      <div class="bg-white rounded-xl border border-slate-200">
        ...
        <div v-if="outOfStock.length > 0" class="divide-y divide-slate-100">
    ...
    <div class="bg-white rounded-xl border border-slate-200">
      <div class="px-5 py-4 border-b border-slate-200 flex items-center justify-between">
        <h3 class="font-semibold text-slate-900 flex items-center gap-2">
          <Icon name="lucide:lock" class="w-4 h-4 text-blue-600" />
          Rezervasyonlar
        </h3>
        <select
          v-model="reservationFilter"
          class="px-2.5 py-1.5 border border-slate-300 rounded-md text-sm bg-white"
        >
        ...
        <table class="w-full">
          <thead class="bg-slate-50 border-b border-slate-200 text-left">
            <tr>
              ...
              <th class="px-5 py-3 text-xs font-semibold text-slate-700 uppercase">Sipariş</th>
              <th class="px-5 py-3 text-xs font-semibold text-slate-700 uppercase">Bayi</th>
              <th class="px-5 py-3 text-xs font-semibold text-slate-700 uppercase">Miktar</th>
              <th class="px-5 py-3 text-xs font-semibold text-slate-700 uppercase">Durum</th>
              <th class="px-5 py-3 text-xs font-semibold text-slate-700 uppercase">Oluşturuldu</th>
            </tr>
          </thead>
          <tbody class="divide-y divide-slate-100">
            <tr v-for="r in reservations" :key="r.id" class="hover:bg-slate-50">
              <td class="px-5 py-3 text-sm text-slate-900 truncate max-w-xs">{{ r.productName }}</td>
              <td class="px-5 py-3 text-sm font-mono text-slate-700">{{ r.orderNo }}</td>
              <td class="px-5 py-3 text-sm text-slate-700">{{ r.dealerName ?? '—' }}</td>
              <td class="px-5 py-3 text-sm font-semibold text-slate-900">{{ formatNumber(r.quantity) }}</td>
              <td class="px-5 py-3"><StatusBadge v-bind="reservationBadge(r.status)" /></td>
              <td class="px-5 py-3 text-xs text-slate-500">{{ formatRelative(r.createdAt) }}</td>
            </tr>
          </tbody>
        </table>
