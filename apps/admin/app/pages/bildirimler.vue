...
<template>
  <div class="space-y-5">
    ...
    <!-- Grouped by product -->
    <div v-if="groupedByProduct.length > 0" class="bg-white rounded-xl border border-ink-200">
      <div class="px-5 py-4 border-b border-ink-200">
        <h3 class="font-semibold text-ink-900">Ürün Bazında Bekleyenler</h3>
      </div>
      <div class="divide-y divide-ink-100">
        ...
        >
          <div>
            <p class="font-medium text-ink-900 text-sm">{{ g.productName }}</p>
            <p class="text-xs text-ink-500">{{ g.count }} kişi bekliyor</p>
          </div>
          <button
    ...
    <div class="bg-white rounded-xl border border-ink-200">
      <div class="px-5 py-4 border-b border-ink-200">
        <h3 class="font-semibold text-ink-900">Tüm Talepler</h3>
      </div>
        ...
        <table class="w-full">
          <thead class="bg-ink-50 border-b border-ink-200 text-left">
            <tr>
              ...
              <th class="px-5 py-3 text-xs font-semibold text-ink-700 uppercase">Kanal</th>
          ...
          <tbody class="divide-y divide-ink-100">
            <tr v-for="n in notifications.filtered" :key="n.id" class="hover:bg-ink-50">
              <td class="px-5 py-3 text-sm text-ink-900 truncate max-w-xs">{{ n.productName }}</td>
              <td class="px-5 py-3 text-sm">
                <p class="text-ink-700">{{ n.email ?? n.phone }}</p>
              </td>
              <td class="px-5 py-3">
                <StatusBadge :variant="n.isDealer ? 'purple' : 'info'" :label="n.isDealer ? 'Bayi' : 'B2C'" />
              </td>
              <td class="px-5 py-3 text-xs text-ink-600">{{ n.channel ?? '—' }}</td>
              <td class="px-5 py-3"><StatusBadge v-bind="statusBadge(n.status)" /></td>
              <td class="px-5 py-3 text-xs text-ink-500">{{ formatRelative(n.createdAt) }}</td>
            </tr>
          </tbody>
        </table>
