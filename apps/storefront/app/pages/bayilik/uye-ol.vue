...
const isFormValid = computed(() => {
  return (
    ...
    formData.value.cariHesapNumarasi.trim() &&
...
const validateField = (field: keyof FormData) => {
  ...
  switch (field) {
    ...
    case 'cariHesapNumarasi':
      if (!value.trim()) {
        newErrors[field] = 'Bu alan zorunludur'
      } else if (!/^[0-9A-Z\-]{3,}$/.test(value.trim())) {
        newErrors[field] = 'Geçerli bir cari hesap numarası girin'
      } else {
        delete newErrors[field]
      }
      break
  ...
  <div class="min-h-screen bg-gradient-to-b from-primary-50 to-white py-12 lg:py-16">
    <div class="px-6 lg:px-12 mx-auto max-w-md">
      ...
      <form @submit.prevent="handleSubmit" class="bg-white rounded-xl shadow-md p-8 space-y-4">
        ...
        <div>
          <label class="block text-sm font-semibold text-primary-900 mb-2">
            Cari Hesap Numarası <span class="text-red-500">*</span>
          </label>
          <input
            v-model="formData.cariHesapNumarasi"
            type="text"
            placeholder="Netsis cari hesap numarası"
            @blur="validateField('cariHesapNumarasi')"
            class="w-full px-4 py-2 border rounded-lg focus:outline-none focus:ring-2 focus:ring-accent-500"
            :class="{ 'border-red-500': errors.cariHesapNumarasi }"
          />
          <p class="text-xs text-ink-500 mt-1">
            Netsis sistemindeki müşteri hesap numaranız
          </p>
          <p v-if="errors.cariHesapNumarasi" class="text-xs text-red-500 mt-1">
            {{ errors.cariHesapNumarasi }}
          </p>
        </div>
        ...
        <p class="text-center text-sm text-ink-600 pt-4">
          ...
          <NuxtLink to="/bayilik/giris" class="text-accent-600 hover:text-accent-700 font-semibold">
            Giriş yapın
          </NuxtLink>
        </p>
      </form>
